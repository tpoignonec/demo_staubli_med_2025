...
public:
  ...
  : Node("trajectory_planner_node")
  {
    ...
    this->declare_parameter("square_size", 0.1);
...
private:
  ...
  bool execute_cartesian_rotations()
  {
    ...
    std::vector<double> rotation_angles = {0.0, M_PI/2, 0.0};
    ...
    for (size_t i = 0; i < rotation_angles.size(); ++i) {
      ...
      // Rotation around Z-axis relative to start pose orientation
      // Combine start pose rotation with additional Z-axis rotation
      tf2::Quaternion q_start;
      tf2::fromMsg(start_pose_.pose.orientation, q_start);

      tf2::Quaternion q_rotation;
      q_rotation.setRPY(0.0, 0.0, angle);  // Pure Z-axis rotation
      ...
      if (!success) {
        RCLCPP_ERROR(this->get_logger(), "Planning failed for rotation %zu", i + 1);
        return false;
      }
      ...
      if (!success) {
        RCLCPP_ERROR(this->get_logger(), "Execution failed for rotation %zu", i + 1);
        return false;
      }
  ...
  bool execute_square_trajectory()
  {
    ...
    for (size_t i = 1; i < square_waypoints.size(); ++i) {
      ...
      if (!success) {
        RCLCPP_ERROR(this->get_logger(), "Planning failed for corner %zu", i);
        return false;
      }
      ...
      if (!success) {
        RCLCPP_ERROR(this->get_logger(), "Execution failed for corner %zu", i);
        return false;
      }
